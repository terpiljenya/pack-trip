...
export default function MapView({ planData, destination }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    const fetchCoords = async () => {
        ...
        cityPlan.day_plans.forEach((dayPlan) => {
          dayPlan.activities.forEach((activity) => {
            ...
            if (activity.location && activity.location.includes('@')) {
              const coordMatch = activity.location.match(/@(-?\d+\.?\d*),(-?\d+\.?\d*)/);
              if (coordMatch) {
      ...
      planData.city_plans.forEach((cityPlan, cityIdx) => {
        ...
        cityPlan.day_plans.forEach((dayPlan, dayIdx) => {
          ...
          dayPlan.activities.forEach((activity, actIdx) => {
            ...
            cleanLocation = cleanLocation.replace(/^(Address:|Location:|At:)\s*/i, '');
            ...
            if (cityPlan.city && !cleanLocation.toLowerCase().includes(cityPlan.city.toLowerCase())) {
              parts.push(cityPlan.city);
            }
            
            // Add destination if available and different from city
            if (destination && destination !== cityPlan.city && !cleanLocation.toLowerCase().includes(destination.toLowerCase())) {
              parts.push(destination);
            }

            const query = parts.join(', ');
            console.log(`MapView: Created query for activity "${activity.name}": "${query}"`);
      ...
      console.log('MapView: All geocoding queries created:', Array.from(queryMap.entries()).map(([query, activities]) => ({
        query,
      ...
      const geocode = async (query: string): Promise<[number, number] | null> => {
        ...
        try {
          ...
          if (data && data.lat && data.lon && !data.error) {
            ...
            if (!isNaN(lat) && !isNaN(lon)) {
              geocodeResultCache[query] = [lat, lon];
              console.log(`MapView: ✅ Successfully geocoded "${query}" to [${lat}, ${lon}]`);
              return [lat, lon];
            }
          }
          ...
          if (simplifiedQuery !== query && simplifiedQuery.length > 0) {
            console.log(`MapView: Trying simplified query: "${simplifiedQuery}"`);
            try {
              await new Promise(resolve => setTimeout(resolve, 200));
              const resp = await fetch(`/api/geocode?q=${encodeURIComponent(simplifiedQuery)}`);
              ...
              if (resp.ok) {
                ...
                if (data && data.lat && data.lon && !data.error) {
                  ...
                  if (!isNaN(lat) && !isNaN(lon)) {
                    ...
                    console.log(`MapView: ✅ Successfully geocoded simplified "${simplifiedQuery}" to [${lat}, ${lon}]`);
                    return [lat, lon];
                  }
                }
              }
            } catch (fallbackErr) {
              console.error('MapView: Fallback geocoding also failed for', simplifiedQuery, fallbackErr);
            }
          }
        }
      ...
      geoResults.forEach(({ query, coords }, index) => {
        console.log(`MapView: Processing geocoding result ${index + 1}/${geoResults.length}: "${query}"`);
        ...
        console.log(`MapView: ✅ Got coordinates [${lat}, ${lon}] for "${query}"`);
        
        if (!first) first = [lat, lon];
        
        const entries = queryMap.get(query)!;
        console.log(`MapView: Creating ${entries.length} markers for "${query}":`, entries);
        
        entries.forEach(({ day, activityName }) => {
          const marker = { position: [lat, lon] as [number, number], day, activityName };
          markersResult.push(marker);
          console.log(`MapView: Added marker for Day ${day}: ${activityName} at [${lat}, ${lon}]`);
        });
      ...
      console.log(`MapView: Created ${markersResult.length} markers from ${geoResults.length} geocoding results`);

      // Fallback: if no markers found, geocode destination only
      if (markersResult.length === 0 && destination) {
        console.log('MapView: No markers found, trying destination fallback:', destination);
        const coords = await geocode(destination);
        if (coords) {
          const [lat, lon] = coords;
          console.log(`MapView: ✅ Destination fallback successful: [${lat}, ${lon}]`);
          markersResult.push({ position: [lat, lon], day: 1, activityName: destination });
          boundsBuilder = new L.LatLngBounds([lat, lon], [lat, lon]);
          if (!first) first = [lat, lon];
        } else {
          console.log(`MapView: ❌ Destination fallback failed, trying known locations`);
          ...
          if (knownLocation) {
            ...
            console.log(`MapView: ✅ Using known location for ${destination}:`, [lat, lon]);
            markersResult.push({ position: [lat, lon], day: 1, activityName: destination });
            boundsBuilder = new L.LatLngBounds([lat, lon], [lat, lon]);
            if (!first) first = [lat, lon];
          } else {
            console.log(`MapView: ❌ No known location found for ${destination}`);
          }
        }
      }
      ...
      console.log('MapView: Final markers summary:', {
        ...
        markersByDay: markersResult.reduce((acc, marker) => {
